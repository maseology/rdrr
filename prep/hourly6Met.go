...
func (c *Cell) Compute6hourly(tC, pPa, rhf, wvel, ccf float64, dt time.Time) (ep float64) {
	...
	for i := 0; i < 6; i++ {
		...
		ep1 := pet.PenmanMonteith(Kg, 0., tC, pPa, rhf, wvel, 0.) // liquid water mass density flux [m/s]
		ep += ep1 * 60. * 60.                                     // [m] (total demand over the hour)
	}
