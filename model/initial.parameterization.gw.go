...
func (dom *Domain) ParameterizeGW(TOPMODELm []float64, dinc float64, prnt bool) ([]*Surface, []int, map[int]int) {
	tt := mmio.NewTimer()

	lus := make([]*Surface, dom.Nc)
	xg := make([]int, dom.Nc) // cross-referencing
	...
	return lus, xg, m // m[dom.Strc.CID0] // -1
}
